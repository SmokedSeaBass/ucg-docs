...
> This guide was written for UCG v1.0.2
...
![](../../../assets/guides/level_editor_objects/land_mine.png)
...
Let's start with duplicating our object's scene (`res://assetsNEW/scenes/objects/gimmicks/land_mine.tscn`), giving it a new name, and placing it in our mod's folder. You can do this by right-clicking the file in Godot and choosing `Move/Duplicate to...`. Following the base game's naming convention (e.g. `water_death.tscn` and `water_deathEditor.tscn`), we will name our new scene `land_mineEditor.tscn`.
...
Now let's open up our newly-duplicated scene. The root "LandMine" node is still pointing to the original `land_mind.gd` script, so let's fix that and change it to `land_mineEditor.gd`. Then, open up our new script.
...
Levels identify objects by their `.tscn` path, so renaming your object's scene file or mod as a whole will also cause issues for pre-existing levels.
:::
...
In the base game, only two entities can interact with the land mine: the player and the horse boss. We will modify our land mine so that Uncanny can also trigger the land mine and be blown up.
...
Testing it out, Uncanny goes over the land mine and... nothing happens?
...
Finally let's add the relevant code. Since this explosion sound only needs to play when *Uncanny* specifically interacts with the land mine, we'll put it here instead of the `explode()` function.
...
- `Name`: This is the variable's label in the level editor (e.g. "Max Speed", "Dir X", etc.). For this guide, we will put "Can Uncanny interact".
- `Varname`: This is the internal script variable that the editor variable is linked to. In our example, this will be "can_uncanny_interact". Be sure to exactly match spelling/case-sensitivity!
...
**Congratulations**, your mod is now feature-complete! Our last big task will be refactoring our changes to be best suited for mod exporting.

## Converting our changes into a mod script

If you've read [mod script basics](/guides/modscript_basics/), you would understand that, due to changes we made directly to `level_editor.tscn`, exporting our mod in its current state could lead to all sorts of compatibility issues with other mods. Let's convert these manual changes into self-contained executable code.

As a reminder, the manual changes we made to the level editor scene were:
- Adding a new `EditorObject` to `levelEditor.objectScenes`
- Adding a new entry to `levelEditor/HUD/tabs/objectsTab/ItemList`
...
Instead of hooking to nodes during active gameplay like in [Node Interception 101](/guides/modscript_basics/#node-interception-101), we will make our mod run during the game's initial startup. Since we want our logic to run just once, we'll put our code in the `_ready()` function.

First, let's (pre)load the level editor `PackedScene` and instantiate it to get access to the packed nodes.
...
One strength of having our changes as GDScript is that we can monitor the important caveat that our land mine has the same entry in both lists. If our `ItemList` and `objectScenes` are different sizes, we know something went wrong.
...
+		"testmod: EditorObject and ItemList entry indices do not match!")
...
:::note
If your custom object is not showing up in the level editor and no debug statements are getting logged, make sure that you have "Mods Enabled" checked in the game settings and that the test mod is activated.

![](../../../assets/guides/level_editor_objects/mods_enabled.png)

User-specific data/settings are shared between all instances of UCG, so for testing be sure to disable any other mods you might have enabled from your personal copy of UCG.
:::

## Conclusion

And with that, your very first custom level editor object mod is complete and ready for exporting! Thank you for following along, and I hope you learned something new!
...
- In the settings menu of UCG, make sure "Mods Enabled" is enabled and that the test mod is enabled.
- If you haven't exported your mod yet, make sure you are launching your game through Godot's `Run Project` button and not via an executable copy of the game
- If you have exported your mod AND are launching the game through Godot, there may be a mod ID conflict between the mod being both in your user directory `mods/` folder and in the Godot project. Consider either:
	- Removing the mod from your user directory `mods/` folder
	- Changing the mod's ID in either the Godot project's `mods.json` or your exported mod's `mods.json`
</details>

<details>
<summary>My custom object is showing up twice in the level editor!</summary>
You likely have a mix of manual scene changes and mod script code. I recommend making a [fresh decompiled copy of UCG](/getting-started/decomp/) and copying your project's `mods/` folder and `mods.json` there. This will undo any changes you made to vanilla assets.
</details>
